...
|---|-----|---------------|---------------|
| 1 | **Certification wording** | The CV currently claims "certifications in AWS (Developer Associate) and Microsoft Azure (AZ-204, AZ-400, AZ-900)". The certificates in `Certificates.pdf` say *"has successfully completed the course in…"* — that is IT Academy coursework, not a passed vendor exam. A Canadian employer who checks AWS's credential verifier and finds nothing treats it as misrepresentation. | Confirm whether any vendor exam was actually sat and passed (credential ID / Credly badge). If not, the CV engine renders all of them as *coursework* — which is still genuinely impressive and completely safe. |
